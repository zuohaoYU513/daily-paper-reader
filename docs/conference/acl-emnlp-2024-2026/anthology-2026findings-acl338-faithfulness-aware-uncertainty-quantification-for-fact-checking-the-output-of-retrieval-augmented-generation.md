...
title_zh: FRANQ：面向检索增强生成输出事实验证的忠实度感知不确定性量化
...
score: 7.0
evidence: 区分事实正确性与检索证据忠实度的RAG幻觉检测方法，属于幻觉量化与RAG研究
tldr: 现有RAG幻觉检测常混淆事实正确性与对检索证据的忠实度，导致事实正确但缺乏检索支持的输出被误判为幻觉。该文提出FRANQ，一种忠实度感知的不确定性量化方法，对RAG输出进行事实验证。它分别刻画模型内部事实错误与检索上下文不一致带来的不确定性，能够更准确地识别真正的幻觉。相关工作为RAG幻觉检测和证据可追溯提供了精细化工具。
...
motivation: RAG幻觉可能来自内部知识或检索上下文，现有方法混淆事实性与忠实度，误判正确陈述为幻觉。
method: 提出FRANQ，将不确定性分解为事实性不确定与忠实度不确定，从而实现忠实度感知的幻觉检测和事实验证。
result: 在开放域问答等RAG场景中，FRANQ比现有幻觉检测方法更准确地区分两类错误并识别真正幻觉。
conclusion: 区分忠实度与事实性能显著提升RAG输出不确定性量化的可解释性与准确性。
---
